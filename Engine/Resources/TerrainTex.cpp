...
{
	// Terrain의 범위가 정해지지 않았을 때 임시로 성공 (초기화)
	if (dwCntX == 0 && dwCntZ == 0 && dwVtxItv == 0) return S_OK;
	...
	for (_ulong i = 0; i < dwCntZ; ++i)
	{
		for (_ulong j = 0; j < dwCntX; ++j)
		{
			...
			pVertex[dwIndex].vTexUV = _vec2(_float(j) / (dwCntX - 1) * m_fDetailX,
				_float(i) / (dwCntZ - 1) * m_fDetailZ);
		}
	}
...
void CTerrainTex::SetInfo(const _ulong & dwCntX, const _ulong & dwCntZ, const _ulong & dwVtxItv, const _float & fDetailX, const _float & fDetailZ)
{
	Safe_Release(m_pVB);
	Safe_Release(m_pIB);
	...
	CTerrainTex::Ready_Buffer(dwCntX, dwCntZ, dwVtxItv);
}

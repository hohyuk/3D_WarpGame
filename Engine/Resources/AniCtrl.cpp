...
void Engine::CAniCtrl::Set_AnimationSet(const _uint& iIndex)
{
	...
	//_double dAnimTimeRate = m_dPeriod;
	//_double dTest = 0.25;					//0.25;
	//// 0번 트랙에 애니메이션 셋 세팅
	...
	//m_pAniCtrl->KeyTrackEnable(m_iCurrentTrack, FALSE, m_fAccTime + dTest * dAnimTimeRate);

	//// 인자값으로 들어오는 트랙에 세팅된 애니메이션 셋을 어떤 속도로 움직일 것인지 설정하는 함수(속도의 상수 값은 1)
	//m_pAniCtrl->KeyTrackSpeed(m_iCurrentTrack, 1.f * _float(dAnimTimeRate), m_fAccTime * _float(dAnimTimeRate), dTest * dAnimTimeRate, D3DXTRANSITION_LINEAR);

	//// 인자값으로 들어오는 트랙의 가중치를 설정하는 함수
	//m_pAniCtrl->KeyTrackWeight(m_iCurrentTrack, 0.1f, m_fAccTime * _float(dAnimTimeRate), dTest * dAnimTimeRate, D3DXTRANSITION_LINEAR);


	//// New 트랙의 활성화를 지시하는 함수
	//m_pAniCtrl->SetTrackEnable(m_iNewTrack, TRUE);
	//m_pAniCtrl->KeyTrackSpeed(m_iNewTrack, 1.f * _float(dAnimTimeRate), m_fAccTime * _float(dAnimTimeRate), dTest * dAnimTimeRate, D3DXTRANSITION_LINEAR);
	//m_pAniCtrl->KeyTrackWeight(m_iNewTrack, 0.9f * _float(dAnimTimeRate), m_fAccTime * _float(dAnimTimeRate), dTest * dAnimTimeRate, D3DXTRANSITION_LINEAR);
...
_bool Engine::CAniCtrl::Is_AnimationSetEnd()
{
	...
	if (tTrackInfo.Position >= m_dPeriod - 0.3)
		return true;

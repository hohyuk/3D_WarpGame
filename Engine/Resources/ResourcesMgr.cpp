...
HRESULT Engine::CResourcesMgr::Ready_Buffer(LPDIRECT3DDEVICE9 pGraphicDev, const _ushort& wContainerIdx, const _tchar* pBufferTag, BUFFERID eID, _ulong dwCntX /*= 1*/, _ulong dwCntZ /*= 1*/, _ulong dwItv /*= 1*/)
{
	...
	case BUFFER_TERRAINTEX:
		pResources = CTerrainTex::Create(pGraphicDev, dwCntX, dwCntZ, dwItv);
		break;
	case BUFFER_CUBETEX:
		pResources = CCubeTex::Create(pGraphicDev);
		break;
	case BUFFER_NAVITRICOL:
		pResources = CNaviTriCol::Create(pGraphicDev);
		break;
	}
...
HRESULT CResourcesMgr::Ready_Terrain_Buffer(LPDIRECT3DDEVICE9 pGraphicDev, const _ushort& wContainerIdx, const _tchar* pBufferTag, const _tchar* pDataPath, _ulong dwCntX, _ulong dwCntZ, _ulong dwItv) {
...
void CResourcesMgr::Adjust_TerrainBuffer(const _ushort & wContainerIdx, const _tchar * pBufferTag, const _ulong & dwCntX, const _ulong & dwCntZ, const _ulong & dwVtxItv, const _float& fDetail)
{
	CResources*		pResources = Find_Resources(wContainerIdx, pBufferTag);
	...
	dynamic_cast<CTerrainTex*>(pResources)->SetInfo(dwCntX, dwCntZ, dwVtxItv, fDetail);
}
...
CVIBuffer* CResourcesMgr::Get_Buffer(const _ushort& wContainerIdx, const _tchar* pBufferTag)
{
	CResources* pResources = Find_Resources(wContainerIdx, pBufferTag);
	if (nullptr == pResources)
		return nullptr;

	return dynamic_cast<CVIBuffer*>(pResources);
}

LPDIRECT3DBASETEXTURE9 CResourcesMgr::Get_Texture(const _ushort& wContainerIdx, const _tchar* pTextureTag, const _uint& iIndex) {
	...
	return dynamic_cast<CTexture*>(pResources)->Get_Texture(iIndex);
}

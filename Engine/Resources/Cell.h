...
public:
	...
	void			Set_Pos(_int _index, const _vec3& vPos) { m_vPoint[_index] = vPos; }
	_float			Get_Height(const _vec3& _vPos);
	_vec3			Get_PosInCell(const _vec3 & _vPos);
	_vec3			GetCenterPoint() const { return m_vCenterPoint; }

	_bool			IsCollide(const _vec3& _vPos);
public:

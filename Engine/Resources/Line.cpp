...
_vec2 CLine::GetPosInLine(const _vec2 & _vPosXZ)
{
	...
	fDotToPosition = D3DXVec2Dot(&m_vNormal, &_vPosXZ);
	// 라인과 원점간 내적을 구한다. (-d)
	fDotToLine = D3DXVec2Dot(&m_vNormal, &m_vPoint[CLine::POINT_START]);

	if (fDotToPosition - fDotToLine > 0.f) {
		// (y =) ax + b => ax -y + b = 0;
		// 점과 직선 사이 거리 공식으로 거리를 구한다.
		_float fLength = fabs(m_fGradient * _vPosXZ.x - _vPosXZ.y + m_fIntercept)
			/ sqrtf(m_fGradient * m_fGradient + 1.f/*-1 * -1*/);
		// 법선 벡터(마름모 안쪽)쪽으로 밀어낸다.
		return _vec2(_vPosXZ.x - m_vNormal.x * fLength, _vPosXZ.y - m_vNormal.y * fLength);
	}

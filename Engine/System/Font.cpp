...
void CFont::Render_Font(const _tchar* pString, const _vec2* pPos, const _vec2* pScale, D3DXCOLOR Color) {
...
void CFont::Render_Font(const _tchar* pString, const _vec2* pPos, DWORD format, D3DXCOLOR Color) {
	//scale 더미임, 삭제해야되는데 보류중...
	RECT	rc{ _long(pPos->x), _long(pPos->y),  _long(pPos->x), _long(pPos->y) };
	m_pSprite->Begin(D3DXSPRITE_ALPHABLEND);
	m_pFont->DrawTextW(m_pSprite, pString, lstrlen(pString), &rc, format, Color);

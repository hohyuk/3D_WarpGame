...
void Engine::CFontMgr::Render_Font(const _tchar* pFontTag, const _tchar* pString, const _vec2* pPos, const _vec2* pScale, D3DXCOLOR Color)
{
	CFont*	pFont = Find_Font(pFontTag);
	NULL_CHECK(pFont);

	pFont->Render_Font(pString, pPos, pScale, Color);

}

void Engine::CFontMgr::Render_Font(const _tchar* pFontTag, const _tchar* pString, const _vec2* pPos, DWORD format, D3DXCOLOR Color)
{
	CFont*	pFont = Find_Font(pFontTag);
	NULL_CHECK(pFont);

	pFont->Render_Font(pString, pPos, format, Color);
